    ...
    if(in_array($fileActualExt, $allowedFileType)) {
        if($fileError === 0 ){
            if($fileSize < 10000000) {
                ...
                include "conn.php";
                $sql = "INSERT INTO listing_t (Name, Type, Breed, Age, Description, Gender, Location, imageurl, SurrenderID) VALUES ('".$petname."', '".$pettype."', '".$petbreed."', '".$petage."', '".$petdescription."','".$petgender."','".$location."','".$imgURL."','".$id."');";
                mysqli_query($conn, $sql);
                
                if (mysqli_affected_rows($conn)>0)
                {
                    echo "<script>alert('Successfully applied. Please await approval from Admin. You can check the approval status in your profile. Thank you for using AWW.') </script>";
                }  else {
