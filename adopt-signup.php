...
if (isset($_POST['regissubmit'])) {
    ...
    $unamecheck = mysqli_query($conn, "SELECT * FROM mem_t WHERE Username= '$username'");
    $emailcheck = mysqli_query($conn, "SELECT * FROM mem_t WHERE Email= '$email'");  
    ...
    if (mysqli_num_rows($unamecheck) > 0 ) {
        ...
        if (mysqli_num_rows($emailcheck) > 0) {
            ...
            $sql = "INSERT INTO mem_t (Username, Password, PhoneNumber, Email, Gender, DateOfBirth) VALUES ('".$username."', '".$password."', '".$phonenum."', '".$email."', '".$gender."', '".$dob."');";
